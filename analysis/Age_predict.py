...
def main():
    ...
    df_gm = df_gm.drop(df_gm[df_gm['Health_Categ'] == 'Medium'].index)
    df_wm = df_wm.drop(df_wm[df_wm['Health_Categ'] == 'Medium'].index)
    df_gm = df_gm.drop(df_gm[df_gm['Health_Categ'] == 'NaN'].index)
    df_wm = df_wm.drop(df_wm[df_wm['Health_Categ'] == 'NaN'].index)

    fig, ax = plt.subplots(figsize=(4, 5), num=None, facecolor='w', edgecolor='k')

    df_data = df_gm
    df_data = df_data.reset_index(drop=True)
    interaction_plot(x=df_data['group'], trace=df_data['Health_Categ'], response=df_data['Norm_Delta_gm'], 
        colors=['black', 'black'], markersize=21, ax=ax, linewidth=7)
    ...
    # Adjust layout
    plt.tight_layout()
    plt.savefig((COV_DATA_DIR + "/figs/interaction_plot_health_gm.png"), format='png', dpi=600)

    fig, ax = plt.subplots(figsize=(4, 5), num=None, facecolor='w', edgecolor='k')

    df_data = df_wm
    df_data = df_data.reset_index(drop=True)
    interaction_plot(x=df_data['group'], trace=df_data['Health_Categ'], response=df_data['Norm_Delta_wm'], 
        colors=['black', 'black'], markersize=21, ax=ax, linewidth=7)
    ...
    # Adjust layout
    plt.tight_layout()
    plt.savefig((COV_DATA_DIR + "/figs/interaction_plot_health_wm.png"), format='png', dpi=600)

    # make_2factors_2levels_design_contrast_cell_means(depvar = 'Norm_Delta_gm', 
    #             f1 = 'group', f1l1 = 'Pandemic', f1l2 = 'No Pandemic',
    #             f2 = 'Health_Categ', f2l1 = 'Low', f2l2 = 'High',
    #             df = df_gm)
    # call_pal('5000')
    ...
    # df_gm = df_gm.drop(df_gm[df_gm['Health_Categ'] == 'High'].index)
    # df_wm = df_wm.drop(df_wm[df_wm['Health_Categ'] == 'High'].index)
    df_gm = df_gm.drop(df_gm[df_gm['Health_Categ'] == 'Low'].index)
    df_wm = df_wm.drop(df_wm[df_wm['Health_Categ'] == 'Low'].index)
    ...
    plt.savefig((COV_DATA_DIR + "/figs/Health_norm_gm_High.png"), format='png', dpi=600)
    ...
    plt.savefig((COV_DATA_DIR + "/figs/Health_norm_gm_Low.png"), format='png', dpi=600)
    ...
    plt.savefig((COV_DATA_DIR + "/figs/Health_norm_wm_High.png"), format='png', dpi=600)
    ...
    plt.savefig((COV_DATA_DIR + "/figs/Health_norm_wm_Low.png"), format='png', dpi=600)
